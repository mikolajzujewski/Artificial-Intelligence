...
class SearchBasedPlayer(Player):
    ...
    def search_path(self, snake: Snake, food: Food, *obstacles: Set[Obstacle]):
        ...
        if AVOID:
            for o in obstacles:
                for p in o:
                    obstaclesPositions.append((p.position.x, p.position.y))
        ...
        possibilities = []
        visits = []
        possibilities.append((head.x, head.y))
        visits.append((head.x, head.y))
        treeDictionary = {}

        for x, y in tail:
            if target.x == x and target.y == y:
                print("Food spawned in tail, no path possible")
                pygame.time.wait(2000)
                exit()

        while possibilities:
            if algo:
                tile = possibilities.pop(0)
            else:
                tile = possibilities.pop()

            adTiles = []
            adTiles.append((tile[0], tile[1] - 1))
            adTiles.append((tile[0] - 1, tile[1]))
            adTiles.append((tile[0], tile[1] + 1))
            adTiles.append((tile[0] + 1, tile[1]))
            for t in adTiles:
                if t not in visits and t not in obstaclesPositions and t not in tail and 0 <= tile[1] < GRID_HEIGHT and 0 <= tile[0] < GRID_WIDTH:
                    possibilities.append(t)
                    visits.append(t)
                    treeDictionary[t] = tile
                    if (tile[0], tile[1]) == (target.x, target.y):
                        break

        previousTile = (target.x, target.y)

        path.append((target.x, target.y))

        headFound = False
        while not headFound:
            if treeDictionary.get(tuple(previousTile)) == None:
                print("No collisionless path possible")
                pygame.time.wait(2000)
                exit()
            previousTile = treeDictionary[previousTile]
            path.insert(0, previousTile)
            self.visited.add(Position(previousTile[0], previousTile[1]))
            if previousTile == (head.x, head.y):
                break

        for s in range(len(path) - 1):
            child = path[s]
            parent = path[s + 1]
            nextMove = (parent[0] - child[0], parent[1] - child[1])
            key = ''
            if nextMove == Direction.RIGHT:
                key = Direction.RIGHT
            elif nextMove == Direction.LEFT:
                key = Direction.LEFT
            elif nextMove == Direction.UP:
                key = Direction.UP
            elif nextMove == Direction.DOWN:
                key = Direction.DOWN
            self.chosen_path.append(key)
